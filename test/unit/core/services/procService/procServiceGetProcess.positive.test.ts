...
import { IProcServiceGetProcessOutputSuccess } from '@app-lcs/interfaces/core/services';
import { Types } from 'mongoose';
import nock from 'nock';

/**
 * procServiceGetProcess.positive
 *
...
describe('procServiceGetProcess', () => {
  ...
  describe('positive', () => {
    ...
    beforeAll(() => {
      ...
      /*
       * Mock a single, 200 process response from the generated url.
       */
     ...
     * PERFORM TEST
     */
    test("should return the expected 'mockReturnValue' for 'stat.payload.process'", async () => {
      /*
       * Arrange
       */
      const processId = id;
      const processExpected = mockReturnValue;
      ...
      const stat = (await procService.getProcess({
        processId,
      })) as IProcServiceGetProcessOutputSuccess;
      const processFound = stat.payload.process;

      /*
       * Assert
       */
      expect(processFound).toEqual(processExpected);
    });
  });
});
