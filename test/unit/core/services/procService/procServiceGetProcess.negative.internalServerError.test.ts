...
/**
 * procServiceGetProcess.negative.internalServiceError
 *
...
describe('procServiceGetProcess', () => {
  ...
  describe('negative.internalServiceError', () => {
    ...
    beforeAll(() => {
      ...
      /*
       * Mock a single, 500 process response from the generated url.
       */
    ...
    test("should return 'stat.error' = true and 'stat.payload.message' = 'response: invalid'", async () => {
       ...
       * Act
       */
      const stat = await procService.getProcess({ processId });
      const { error, payload } = stat as IProcServiceGetProcessOutputError;
