...
/**
 * personaServiceGetOrganisation.negative.wrongResponse
 *
...
describe('personaServiceGetOrganisation', () => {
  ...
  describe('negative.wrongResponse', () => {
    ...
    beforeAll(() => {
      ...
      /*
       * Mock a single 200 organisation response from
       * the generated url with 'unknownField',
       * rather than the expected 'organisation' field.
       */
     ...
     * PERFORM TEST
     */
    test("should return the expected error when the 'organisation' field can not be found", async () => {
      /*
       * Arrange
       */
      const organisationId = id;
      const errorExpected = true;
      const messageExpected = 'json[organisation]: not found';
