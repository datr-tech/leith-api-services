...
/**
 * entityServiceHasResource.positive
 *
 * A single test of 'entityService.hasResource', where
 ...
 * @version 0.4.1
 */
describe('entityServiceHasResource', () => {
  let id;
  let mockResponse;
  describe('positive', () => {
    /*
    ...
    beforeAll(() => {
       ...
       * itself, being passed, ultimately, to 'hasResource' as
       ...
       */
      nock(url).get('').times(1).reply(200, { resource: mockResponse });
    });
    afterAll(() => {
      nock.cleanAll();
    });
    ...
    test('should return the expected positive response', async () => {
       ...
       * Act
       */
      const responseFound = await entityService.hasResource({ resourceId });
